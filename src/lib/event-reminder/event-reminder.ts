...
export async function printMonthCelebrants() {
  try {
    ...
    const familyMembers = await readFamilyMembersFromCSV('data/family_members.csv');
    const celebs = getTodayCelebrants(familyMembers);
    if (celebs && celebs.birthdays && celebs.birthdays.length === 0 && celebs.anniversaries && celebs.anniversaries.length === 0) {
      const { birthdays, anniversaries }  = celebs;
